...
(({LETTER}+({LETTER}|{DIGIT})*([_]+({LETTER}|{DIGIT})+)?)[_]) {printf("Error at line %d, column %d,: identifier \"%s\" cannot end with an underscore\n", currLine, currPos, yytext); exit(0);}
	...
	/*check for invalid identifier - must begin with letter*/
(({DIGIT}|[_])({LETTER}+({LETTER}|{DIGIT})*([_]+({LETTER}|{DIGIT})+)?)) {printf("Error at line %d, column %d,: identifier \"%s\" must begin with a letter\n", currLine, currPos, yytext); exit(0);}
